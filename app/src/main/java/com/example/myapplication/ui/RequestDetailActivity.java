...
import com.example.myapplication.model.ReviewRequestRequest;
...
public class RequestDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        role = pm.getRole();
        currentEmployeeId = pm.getEmployeeId();
        ...
        // Hiển thị thông tin
        ((TextView) findViewById(R.id.tvDetailEmployeeName)).setText(employeeName != null ? employeeName : "—");
        ((TextView) findViewById(R.id.tvDetailDept)).setText(departmentName != null ? departmentName : "—");
        ((TextView) findViewById(R.id.tvDetailTitle)).setText(title != null ? title : "—");
        ((TextView) findViewById(R.id.tvDetailDescription)).setText(description != null && !description.isEmpty() ? description : "Không có mô tả");
        ((TextView) findViewById(R.id.tvDetailCreatedAt)).setText(formatDateTime(createdAt));
        ...
        if (!"PENDING".equalsIgnoreCase(status) && reviewedByName != null) {
        ...
        if ("REJECTED".equalsIgnoreCase(status) && rejectionReason != null && !rejectionReason.isEmpty()) {
        ...
        if ("PENDING".equalsIgnoreCase(status) && ("ADMIN".equals(role) || "MANAGER".equals(role))) {
            layoutActions.setVisibility(View.VISIBLE);
            findViewById(R.id.btnApprove).setOnClickListener(v -> approveRequest());
    ...
    private void approveRequest() {
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Duyệt", (d, w) -> {
                    apiService.reviewRequest(requestId, currentEmployeeId, new ReviewRequestRequest(true, null))
                            .enqueue(new Callback<Request>() {
                                @Override
                                public void onResponse(Call<Request> c, Response<Request> r) {
                                    if (r.isSuccessful()) {
                                        Toast.makeText(RequestDetailActivity.this, "Đã duyệt", Toast.LENGTH_SHORT).show();
                                        setResult(RESULT_OK);
                                        finish();
                                    } else {
                                        Toast.makeText(RequestDetailActivity.this, "Lỗi khi duyệt", Toast.LENGTH_SHORT).show();
                                    }
                                }
                                @Override
                                public void onFailure(Call<Request> c, Throwable t) {
                                    Toast.makeText(RequestDetailActivity.this, "Lỗi kết nối", Toast.LENGTH_SHORT).show();
                                }
                            });
                })
    ...
    private void showRejectDialog() {
        final TextInputEditText etReason = new TextInputEditText(this);
        ...
        dialog.setOnShowListener(d -> {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
                ...
                dialog.dismiss();
                apiService.reviewRequest(requestId, currentEmployeeId, new ReviewRequestRequest(false, reason))
                        .enqueue(new Callback<Request>() {
                            @Override
                            public void onResponse(Call<Request> c, Response<Request> r) {
                                if (r.isSuccessful()) {
                                    Toast.makeText(RequestDetailActivity.this, "Đã từ chối", Toast.LENGTH_SHORT).show();
                                    setResult(RESULT_OK);
                                    finish();
                                } else {
                                    Toast.makeText(RequestDetailActivity.this, "Lỗi thao tác", Toast.LENGTH_SHORT).show();
                                }
                            }
                            @Override
                            public void onFailure(Call<Request> c, Throwable t) {
    ...
    private String formatDateTime(String iso) {
        if (iso == null) return "—";
        try {
            // "2026-03-25T14:20:00" -> "25/03/2026 14:20"
            String datePart = iso.length() >= 10 ? iso.substring(8, 10) + "/" + iso.substring(5, 7) + "/" + iso.substring(0, 4) : iso;
            String timePart = iso.contains("T") && iso.length() >= 16 ? " " + iso.substring(iso.indexOf("T") + 1, iso.indexOf("T") + 6) : "";
